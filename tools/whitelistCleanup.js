...
class WhitelistCleanup {
    ...
    async cleanupSecurityManager() {
        try {
            ...
            if (config.security) {
                const originalAdminCount = config.security.admin_numbers?.length || 0;
                const originalGroupCount = config.security.authorized_groups?.length || 0;
                ...
                console.log(`🗑️ 安全管理器清理完成: 移除 ${originalAdminCount} 個管理員, ${originalGroupCount} 個授權群組`);
            }
