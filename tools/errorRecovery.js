class ErrorRecovery {
    ...
    async handleConnectionError(error, client) {
        const errorRecord = this.recordError(error, { action: 'connection_error' });
    ...
    async sendAuthErrorNotification(errorRecord) {
        try {
            ...
            console.error('❌ 發送認證錯誤通知失敗:', notificationError.message);
        }
    }
    ...
    cleanupOldErrors(daysToKeep = 30) {
        ...
        if (removedCount > 0) {
            console.log(`🗑️ 已清理 ${removedCount} 條 ${daysToKeep} 天前的錯誤記錄`);
        }
    }
