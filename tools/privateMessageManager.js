class PrivateMessageManager {
    ...
    async startPrivateSession(userId, originId, client) {
        try {
            ...
            if (this.activeSessions.has(userId)) {
                ...
                if (
                    Date.now() - existingSession.startTime <
                    this.sessionTimeout
                ) {
                    return {
                        success: false,
    ...
    async handlePrivateReply(userId, message, client) {
        try {
            ...
            if (session.step === 'ask_password') {
                return await this.handlePasswordVerification(
                    userId,
                    message,
                    session,
                    client
    ...
    async sendPrivateMessage(userId, message, client) {
        try {
            console.log(
                `🔍 嘗試向 ${userId} 發送私信: ${message.substring(0, 50)}...`
            );
    ...
    getSessionStats() {
        ...
        return {
            ...
            sessions: Array.from(this.activeSessions.values()).map(
                (session) => ({
                    userId: session.userId,
                    originId: session.originId,
                    step: session.step,
