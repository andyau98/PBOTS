...
**症狀**：群組內其他用戶錯誤回答，影響發起人流程；用戶回答後無明確反饋
**解決方案**：實作私信提問機制與私信確認回覆
...
if (logicEngine.isUserInSession(context.userId, context.originId)) {
    ...
    if (context.isGroup) {
        if (
            logicEngine.isGroupSessionLocked(context.originId, context.userId)
        ) {
            // 群組已鎖定給其他用戶，忽略此訊息
            console.log(
                `⛔ 群組 ${context.originId} 已鎖定，忽略用戶 ${context.pushname} 的訊息`
            );
...
// 如果是回答後的私信確認回覆
if (userInput && userId) {
    const confirmationMessage = `收到: ${userInput}`;
    return {
        type: 'private_question_with_confirmation',
...
1. 用戶在群組發起指令，Bot 私下發送第一個問題
2. 用戶在私信中回答，Bot 在私信中發送確認：`收到: 啟德`
3. Bot 在私信中繼續下一問題
4. 所有問題完成後，Bot 在群組中發布最終結果
5. 解決群組干擾與用戶「無反應」恐懼
...
**創新點**：私信提問與確認回覆，避免群組干擾，解決「無反應」恐懼
**實現方式**：`private_question_with_confirmation` 類型，私信訊息發送
