...
client.on('ready', async () => {
    ...
    console.log(`📄 圖片轉PDF: ${config.media_download?.enabled ? '✅ 已啟用' : '❌ 已禁用'}`);
...
async function handlePrivateReply(message, context) {
    try {
        console.log(`🔐 處理私信回覆來自 ${context.userId}: ${context.messageBody}`);
        
        const result = await privateMessageManager.handlePrivateReply(context.userId, message, client);
...
async function handleCommand(command, message, context) {
    ...
    // 完全開放命令，不受白名單攔截
    const openCommands = ['whitelist', 'help', 'ping', 'status', 'stats', 'weather', '天氣', 'news', '新聞', 'construction', '地盤', 'monitor', '監控', 'accident', '意外', 'topdf', 'done', 'cancel'];
    if (!openCommands.includes(command)) {
        if (!userPermission.hasFullAccess) {
            await message.reply(`🚫 權限不足！您無法使用此命令。\n使用 !whitelist 獲取管理員權限。`);
    ...
    switch (command) {
        ...
        case 'help':
            const helpText = whitelistManager.generateHelpMessage(context.userId);
        ...
        case 'security':
            ...
            const securityText = securityManager.formatSecurityStatus(securityStatus);
        ...
        case 'cleanup':
            ...
            try {
                ...
                const cleanupText = cleanupManager.formatCleanupResults(cleanupResults);
        ...
        case 'topdf':
            ...
            try {
                ...
                if (!pdfTitle) {
                    ...
                    // 有標題 → 直接開始收集
                    const result = imageToPdf.setTitle(context.userId, pdfTitle);
                    if (result) {
                        await message.reply(result.message);
                    } else {
        ...
        case 'done':
            ...
            if (!imageToPdf.hasSession(context.userId)) {
                await message.reply('❌ 您沒有進行中的 PDF 收集會話。請先用 #TOPDF [標題] 開始。');
                return;
            }
            try {
                ...
                try {
                    ...
                    console.log(`✅ PDF已發送: ${result.fileName} (${result.imageCount}張)`);
                } catch (sendError) {
                    console.error(`❌ PDF發送失敗: ${sendError.message}`);
                    await message.reply(`⚠️ PDF已生成但發送失敗: ${sendError.message}\n文件路徑: ${result.pdfPath}`);
                }
            } catch (error) {
        ...
        case 'whitelist':
            // 私信問答認證系統（Phase 7機制）
            console.log(`🔐 !whitelist 指令啟動私信問答，來自 ${context.userId}`);
            
            try {
                // 檢查是否已有活躍會話
                if (privateMessageManager.hasActiveSession(context.userId)) {
                    const sessionStatus = privateMessageManager.getSessionStatus(context.userId);
                    await message.reply(`⏰ 您已有一個活躍的認證會話，請檢查私訊。\n剩餘時間: ${sessionStatus.remainingMinutes} 分鐘`);
                    return;
                }
        ...
        case '天氣':
            ...
            try {
                ...
                const weatherResult = await weatherReporter.getCompleteWeatherReport();
        ...
        default:
            await message.reply(`❌ 未知命令: ${command}\n使用 ${prefix}help 查看可用命令。`);
...
client.on('message', async (message) => {
    try {
        ...
        // === PDF 收集會話：攔截照片 ===
        if (message.hasMedia && message.type === 'image' && imageToPdf.hasSession(context.userId)) {
            try {
                const media = await message.downloadMedia();
                if (media) {
                    const buffer = Buffer.from(media.data, 'base64');
                    const caption = message.body || '';
                    const fileName = message.filename || `photo_${Date.now()}.jpg`;
                    const count = await imageToPdf.addPhoto(context.userId, buffer, fileName, caption);
                    await message.reply(`✅ 已收到第 ${count} 張照片` +
                        (caption ? `\n📝 說明: ${caption}` : '') +
                        `\n\n繼續發送照片，或發送 *#done* 生成 PDF。`);
                    console.log(`📸 PDF照片 #${count} 來自 ${context.userId}: ${caption}`);
                }
            } catch (error) {
        ...
        // 檢查是否為私信回覆（處理私信問答）
        if (!isGroup && privateMessageManager.hasActiveSession(context.userId)) {
        ...
        if (messageBody.startsWith(prefix)) {
            // 標準前綴命令
            command = messageBody.slice(prefix.length).toLowerCase().split(' ')[0];
        ...
        if (isGroup && !config.features?.reply_in_group && command !== 'whitelist') {
            console.log(`⏸️ 群組回覆已禁用，忽略命令: ${command}`);
            return;
        }
        
        console.log(`🔄 處理 ${command} 命令來自 ${sourcePrefix} ${senderInfo.pushname}`);
        ...
        errorRecovery.recordError(error, {
            ...
            messageBody: typeof messageBody !== 'undefined' ? messageBody : 'undefined'
        });
...
client.on('disconnected', async (reason) => {
    ...
    errorRecovery.recordError(error, { action: 'disconnected', reason: reason });
